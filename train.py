...
def run_epoch(dataloader, model, loss_compute):
    ...
    tokens = 0
    bertscore = BERTScore()
    for i, (imgs, labels_y, labels) in tqdm(enumerate(dataloader)):
        ...
        if i % 50 == 1:
            ...
            pprint(bertscore(out, batch.ntokens))
...
def train():
    batch_size = 64
    train_dataloader = torch.utils.data.DataLoader(ListDataset(['train_label.txt']), batch_size=batch_size, shuffle=True, num_workers=0)
    val_dataloader = torch.utils.data.DataLoader(ListDataset('test_label.txt'), batch_size=batch_size, shuffle=False, num_workers=0)
